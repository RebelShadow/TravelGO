...
class AdminSecurity:
    ...
    def delete_account(self, acct):
        if acct.is_commited:
            query = REMOVE_USER % {"user":acct.username, "host":acct.host}
...
class AdminAccount:
    ...
    def save(self):
        ...
        if self.is_commited and (self.username != self._orig_username or self.host != self._orig_host):
            if (self.username, self.host) in self._owner.account_names:
                raise WBSecurityValidationError("The '%s'@'%s' account already exists and cannot be saved." % (self.username, self.host))
    ...
    def load(self, username, hostname):
        ...
        query = GET_ACCOUNT_QUERY % {"user":escape_sql_string(username),"host":hostname}
        try:
            result = self._owner.ctrl_be.exec_query(query)
        except Exception, e:
            raise Exception("Error querying security information: %s" % e)
        ...
        query = GET_ACCOUNT_MYSQL_TABLE_PRIVS_QUERY % {"user":escape_sql_string(username),"host":hostname}
        try:
            result = self._owner.ctrl_be.exec_query(query)
        except Exception, e:
